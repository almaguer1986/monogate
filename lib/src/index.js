...
/**
 * The DEML operator: deml(x, y) = exp(−x) − ln(y)
 * Dual gate: deml(x, 1) = exp(−x) — the negative-exponent primitive.
 *
 * @param {number} x
 * @param {number} y  must be > 0 (argument of ln)
 * @returns {number}
 */
export const deml = (x, y) => Math.exp(-x) - Math.log(y);
 ...
 * @returns {number}
 */
export const exp_neg = (x) => deml(x, 1);
...
export const IDENTITIES = [
  ...
  // DEML family — dual gate: deml(x,y) = exp(−x) − ln(y)
  { name: "e^(−x)", operator:"DEML", form: "deml(x,1)",                   nodes: 1,  depth: 1, status: "verified" },
  { name: "ln x",   operator:"DEML", form: "eml(1,eml(eml(1,x),1))",     nodes: 3,  depth: 3, status: "verified" },
];
